...
(function(root, factory){
    ...
    */
            ...
            var self = this,
                ...
                scrollFunc = function(ev) {
                    var $this = $(this),
                        ...
                        height = $this.height(),
                        delta = (ev.type == 'DOMMouseScroll' ? ev.originalEvent.detail * scrollNotch2PixelRatio : ev.originalEvent.wheelDelta),
                        up = delta > 0,
                        prevent = function() {
                            ev.stopPropagation();
                            ev.preventDefault();
    ...
    MultiSelleckt.prototype.toggleDisabled = function(){
        var hasFooter = this.$items.find('footer').length,
            noItemsLeft = this.selectedItems.length === this.items.length;

        this.$sellecktEl.toggleClass('noitems', noItemsLeft);
        this.$sellecktEl.toggleClass('disabled', noItemsLeft && !hasFooter);
    };
